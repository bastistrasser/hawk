...
from hawk.data_profiling.base_types import FeatureType
...
def generate_workflow_overview(preprocessing_steps: list[dict]) -> str:
    ...
    dataset_image = generate_image_from_file(
        os.path.join(current_path, "assets", "dataset.png")
    )
    preprocessing_step_image = generate_image_from_file(
        os.path.join(current_path, "assets", "preprocessing_step.png")
    )
    overview = ""
    for index, step in enumerate(preprocessing_steps):
        if index < len(preprocessing_steps) - 1:
            overview += dataset_template.render(
                dataset_id=step["input"], image=dataset_image
            )
            overview += preprocessing_step_template.render(
                description=step["description"], image=preprocessing_step_image
            )
        else:
            overview += dataset_template.render(
                dataset_id=step["input"], image=dataset_image
            ...
            overview += dataset_template.render(
                dataset_id=step["output"], image=dataset_image
            )
    return overview
...
def generate_column_information(dataset_id: str, data_profile: DataProfile):
    ...
    categorical_headers, categorical_column_data = \
        get_column_data_for_view(categorical_columns)
    other_headers, other_column_data = \
        get_column_data_for_view(other_columns)
    template = env.get_template("column_information.html")
    return \
        template.render(
            column_type="Numeric",
            headers=numeric_headers, 
            columns=numeric_column_data) + \
        template.render(
            column_type="Categorical", 
            headers=categorical_headers, 
            columns=categorical_column_data) + \
        template.render(
            column_type="Other", 
            headers=other_headers, 
            columns=other_column_data)
